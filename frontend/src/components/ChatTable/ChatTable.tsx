...
export default function ChatTable(props: ChatTableProps) {
    ...
    React.useEffect(() => {
        request().post("db-entries", { uuids: Object.keys(props.referenceIds) }).then((res) => {
            setEntries(res.data)
      ...
      <TableContainer sx={{ maxHeight: 500 }}>
        <Table stickyHeader aria-label="sticky table">
          ...
          <TableBody>
            {entries.map((row) => {
                return (
                  <S.StyledTableRow hover role="checkbox" tabIndex={-1} onClick={() => window.location.href = row.url}>
                    {columns.map((column) => {
                      const value = row[column.id];
                      return (
                        <TableCell key={column.id} align={column.align}>
                            <Typography variant="subtitle2" noWrap width={column.width} maxWidth={column.maxWidth}>
